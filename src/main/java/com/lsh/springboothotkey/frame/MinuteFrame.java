...
public class MinuteFrame extends JDialog {
	...
	public MinuteFrame(TaskFrame parent, boolean modal) {
		super(parent, modal);
		Contains.window = this;
		textField = parent.getjTextField6();
		minuCron = textField.getText().trim();
		initComponents();
	}
	...
	private void initComponents() {
		...
		jRadioButton1 = new JRadioButton("每分钟");
		...
		jLabel4 = new JLabel("分内");
		jTextField3 = new JTextField();
		jTextField4 = new JTextField();
		jLabel5 = new JLabel("分开始，每");
		jLabel6 = new JLabel("分钟执行一次");
	...
	private void init() {
		if (minuCron.length() > 0) {
			if ("*".equals(minuCron)) {
				jRadioButton1.setSelected(true);
			}
			if (minuCron.contains("-")) {
				jRadioButton2.setSelected(true);
				String[] zq = minuCron.split("-");
			...
			if (minuCron.contains(",")) {
				...
				String[] other = minuCron.split(",");
				for (int i = 0; i < other.length; i++) {
					SwingUtil.selectBox(this,other[i]);
				}
			}
			if (StringUtils.isNumeric(minuCron)){
				jRadioButton4.setSelected(true);
				SwingUtil.selectBox(this,minuCron);
			}
		} else {
			jRadioButton1.setSelected(true);
		}
